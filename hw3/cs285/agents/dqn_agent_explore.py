...
class DQNAgent(nn.Module):
    def __init__(
        ...
        use_softmax_exploration: bool = False,
        temperature: float = 1.0,  # Temperature parameter for softmax
    ):
        super().__init__()
        ...
        self.use_softmax_exploration = use_softmax_exploration  # New attribute
        self.temperature = temperature  # New attribute
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        # action = ...
        if self.use_softmax_exploration:
            q_values = self.critic(observation).detach()
            softmax_probs = torch.nn.functional.softmax(q_values / self.temperature, dim=1)
