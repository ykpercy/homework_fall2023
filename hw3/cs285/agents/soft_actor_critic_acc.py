...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            swapped_qs = []
            for i in range(num_critic_networks):
                j = (i + 1) % num_critic_networks
                swapped_qs.append(next_qs[j])
            next_qs = torch.stack(swapped_qs, dim=0)
        elif self.target_critic_backup_type == "min":
            min_q, _ = torch.min(next_qs, dim=0)
            next_qs = min_q.unsqueeze(0).expand(num_critic_networks, -1)
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # print(f"the shape of next_qs: {next_qs.shape}")

            if self.use_entropy_bonus and self.backup_entropy:
                # TODO(student): Add entropy bonus to the target values for SAC
                # next_action_entropy = -next_action_distribution.log_prob(next_action).mean(dim=-1)
                ...
                # print(f"the shape of next_action_log_probs: {next_action_entropy.shape}")
                # print(f"the shape of unsqueeze next_action_entropy: {next_action_entropy.unsqueeze(0).shape}")
                # next_action_entropy = -next_action_log_probs.sum(dim=-1)  # Sum across action dimensions
                next_qs = next_qs + self.temperature * next_action_entropy.unsqueeze(0).expand_as(next_qs)

            # Compute the target Q-value
            target_values: torch.Tensor = reward.unsqueeze(0).expand(self.num_critic_networks, -1) + self.discount * (1.0 - done.float().unsqueeze(0).expand(self.num_critic_networks, -1)) * next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        """
        # For continuous actions, we can compute the entropy analytically for many distributions
        # if hasattr(action_distribution, "entropy"):
        #     return action_distribution.entropy().mean(dim=-1)  # Mean across action dimensions
        # TODO(student): Compute the entropy of the action distribution.
        # Note: Think about whether to use .rsample() or .sample() here...
        # else:
            # Fallback to Monte Carlo estimate
        # 蒙特卡洛估计（适用于无解析熵的分布）
        # num_mc_samples = 10  # 可配置的采样次数
        # samples = action_distribution.rsample((num_mc_samples,))  # (num_samples, B, D)
        samples = action_distribution.rsample()
        log_probs = action_distribution.log_prob(samples)  # (B,), 已经是样本上的概率和
        # print(f"the shape of log_probs: {log_probs.shape}")
        # return -log_probs.mean()
        return -log_probs
        # return -log_probs.mean(dim=0)  # (B,)
        # return -log_probs.sum(dim=-1)  # 在动作维度上求和
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        # loss = -(log_probs * advantage.squeeze()).mean()
        # print(f"the shape of actor log_probs: {log_probs.shape}")
