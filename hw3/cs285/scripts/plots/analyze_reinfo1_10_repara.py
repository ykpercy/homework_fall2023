...
def plot_combined_eval_returns(data_dict, output_dir):
    ...
    for algorithm_name, data in data_dict.items():
        ...
        for tag in eval_tags:
            ...
            if steps and values:
                plt.plot(steps, values, linewidth=2, color=colors[color_idx % len(colors)], 
                         label=f"{algorithm_name}: {tag}",
                         linestyle=linestyles[color_idx % len(linestyles)],
                         marker=markers[color_idx % len(markers)], markersize=4, markevery=max(1, len(steps)//20))
                ...
                if len(steps) > 5:  # Only add trend if we have enough data points
                    ...
                    plt.plot(steps_smooth, values_smooth, linewidth=3, 
    ...
    plt.title("Comparison of Evaluation Returns: Reinforce1 vs Reinforce10 vs Reparametrize", fontsize=16, fontweight='bold')
    ...
    # Save figure
    filename = os.path.join(output_dir, "HalfCheetah_eval_returns_comparison_reinfo1_10_repara.png")
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved combined evaluation return plot to {filename}")

def main():
    # Set directories for all three algorithms
    ...
    logdir_reparametrize = './data/hw3_sac_reparametrize_HalfCheetah-v4_reparametrize_s128_l3_alr0.0003_clr0.0003_b128_d0.99_t0.1_stu0.005_14-04-2025_23-41-55'
    ...
    # Extract data for Reparametrize
    print(f"Reading data from Reparametrize: {logdir_reparametrize}")
    data_reparametrize = extract_data(logdir_reparametrize)
    if data_reparametrize:
        all_data['Reparametrize'] = data_reparametrize
    else:
        print("No data found or error reading tfevents files for Reparametrize.")
    ...
    # Plot evaluation returns from all algorithms together
